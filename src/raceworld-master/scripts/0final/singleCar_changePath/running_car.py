...
def follow_line_in(image):
    ...
    cx = int((cx1-cx2)*46/64+cx2)

    cv2.circle(image, (cx, cy), 20, (0, 0, 255), -1)
    err = cx - w / 2 - 60
    twist = Twist()
    twist.linear.x = 0.7
    twist.angular.z = -float(err / 0.8) / 50
...
def do_match(frame):
    max = 0.0
    template = cv2.imread(r"/home/shengzi/aliyun_demo/src/raceworld-master/scripts/0final/singleCar_changePath/01.png")
