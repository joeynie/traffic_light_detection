...
def follow_line_in(image):
    ...
    cx = int((cx1-cx2)*48/64+cx2)

    cv2.circle(image, (cx, cy), 20, (0, 0, 255), -1)
    err = cx - w / 2 - 63
    twist = Twist()
    twist.linear.x = 0.6
    twist.angular.z = -float(err / 0.88) / 50
...
def get_sign(image):
    max = 0.0
    for i in range(1,3):
        template = cv2.imread(r"/home/xiang/ros_project/race_ws/src/raceworld/scripts/words_detection/0"+str(i)+".png")
