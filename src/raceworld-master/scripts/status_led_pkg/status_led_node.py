...
class StatusLedNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("status_led_node started") # 启动节点
    ...
    def set_led_solid_callback(self, req, res):
        ...
        solid服务的回调函数
        参数:
            req (SetStatusLedSolidSrv.Request): 请求对象
            res (SetStatusLedSolidSrv.Response): 响应
        Returns（返回值）:
    ...
    def get_led(self, led_index):
        ...
        if self.leds[led_index] is None:
            led = led_control.LEDControl(led_index, self.get_logger()) # 创建一个LED_control对象
            led.enable()
